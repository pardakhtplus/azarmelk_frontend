...
export default function RangeSlider({
  ...
  const handleMinChange = (value: string) => {
    if (value === "") {
      ...
      if (!isNaN(numValue)) {
        // Allow typing any value in input (unrestricted)
        setLocalMinValue(numValue);
        // Pass the actual value to parent, not clamped
  ...
  const handleMaxChange = (value: string) => {
    if (value === "") {
      ...
      // Don't call onMaxChange to keep the slider enabled
    } else {
      const numValue = parseInt(value);
      if (!isNaN(numValue)) {
        // Allow typing any value in input (unrestricted)
  ...
  // Slider values are always clamped to min/max range, regardless of input values
  const rangeValues = [
    isMinDisabled ? min : Math.max(min, Math.min(localMinValue || min, max)),
    isMaxDisabled ? max : Math.max(min, Math.min(localMaxValue || max, max)),
  ];
