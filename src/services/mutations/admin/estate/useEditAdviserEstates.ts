...
export default function useEditAdviserEstates() {
  const editAdviserEstates = useMutation({
    mutationKey: ["editAdviserEstates"],
    mutationFn: async (props: {
      newAdviserId: string;
      oldAdviserId: string;
    }) => {
      try {
        const res = await api.admin.estate.editAdviserEstate(props);
        if (res.status >= 400) {
