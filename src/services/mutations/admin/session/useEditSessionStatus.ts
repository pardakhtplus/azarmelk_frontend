...
export default function useEditSessionStatus() {
  const editSessionStatus = useMutation({
    mutationKey: ["session", "editSessionStatus"],
    mutationFn: async (props: {
      ...
      try {
        const res = await api.admin.session.editStatus(props);
        if (res.status >= 400) {
