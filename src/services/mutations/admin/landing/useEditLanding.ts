...
export default function useEditLanding() {
  const editLanding = useMutation({
    mutationKey: ["landing", "editLanding"],
    mutationFn: async (props: { id: string; data: TMutateLanding }) => {
      try {
        const res = await api.admin.landing.edit(props.id, props.data);
        if (res.status >= 400) {
