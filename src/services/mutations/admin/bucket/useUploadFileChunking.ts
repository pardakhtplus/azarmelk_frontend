...
export default function useUploadFileChunking() {
  const [isUploading, setIsUploading] = useState(false);
  const [uploadingProgress, setUploadingProgress] = useState(0);
  const abortControllerRef = useRef<AbortController | null>(null);

  const uploadFileChunking = useMutation({
    mutationKey: ["uploadFileChunking"],
    mutationFn: async (props: { file: File }) => {
      try {
        ...
        const initialRes = await api.admin.bucket.uploadChunkFiles({
          index: 0,
        ...
        for (let i = 0; i < totalChunks; i++) {
          ...
          const res = await api.admin.bucket.uploadChunkFiles({
            index: i,
            totalChunks: totalChunks,
            uploadId: uploadId,
            key: key,
  ...
  const cancelUpload = useCallback(() => {
    ...
    if (uploadFileChunking.isPending) {
      console.log("Resetting mutation");
      uploadFileChunking.reset();
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [uploadFileChunking.isPending]);
