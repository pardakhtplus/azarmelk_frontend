...
export default function useUploadWatermarkedFileChunking({
  id,
}: {
  id?: string;
}) {
  ...
  const uploadWatermarkedFileChunking = useMutation({
    mutationKey: ["uploadWatermarkedFileChunking", id],
    mutationFn: async (props: { file: File }) => {
      try {
        ...
        const initialRes = await api.admin.bucket.uploadWaterMarkedChunkFiles({
          index: 0,
        ...
        for (let i = 0; i < totalChunks; i++) {
          ...
          const res = await api.admin.bucket.uploadWaterMarkedChunkFiles({
            index: i,
            totalChunks: totalChunks,
            uploadId: uploadId,
            key: key,
  ...
  const cancelUpload = useCallback(() => {
    ...
    if (uploadWatermarkedFileChunking.isPending) {
      console.log("Resetting mutation");
      uploadWatermarkedFileChunking.reset();
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [uploadWatermarkedFileChunking.isPending]);
