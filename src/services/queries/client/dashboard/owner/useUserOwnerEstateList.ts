...
export function useUserOwnerEstateList({
  phoneNumber,
  categoryId,
  enabled = true,
}: {
  ...
  const userOwnerEstateList = useQuery({
    queryKey: ["userOwnerEstateList", phoneNumber],
    enabled,
    queryFn: async () => {
      try {
        const res = await api.client.dashboard.owner.getEstateList({
          phoneNumber,
          categoryId,
        });
