          ...
          {/* {selectedRegions.map((item) => (
            <span
              ...
              <p className="flex items-center gap-1">
                {item.parent && (
                  <>
                    <span className="opacity-70">{item.parent?.title}</span>
...
//   const handleOptionSelect = (selectedOption: THierarchicalOption) => {
//     const isSelected = selectedRegions.some(
//       (item) => item.id === selectedOption.key,
//     );
//     let newValue: TSelectedOption[];
...
//         newValue = newValue.filter(
//           (item) =>
//             !selectedOption.children?.some((child) => child.key === item.id),
//         );
//       }
//     } else {
//       // Add if not selected
//       newValue = [...selectedRegions, { id: selectedOption.key }];
//       // If this is a parent, also add all children
//       if (selectedOption.children) {
//         const childrenOptions = selectedOption.children.map((child) => ({
//           id: child.key,
...
//   const isOptionSelected = (option: THierarchicalOption) => {
//     return selectedRegions.some((item) => item.id === option.key);
//   };

//   const getSelectedChildrenCount = (option: THierarchicalOption) => {
//     return selectedRegions.filter((item) => item.id === option.key).length;
//   };
...
//         <div className="flex flex-1 flex-wrap gap-2">
//           <span>{placeholder}</span>
//           <span className="flex size-5 items-center justify-center rounded-full bg-primary-blue/10 text-sm text-primary-blue">
//             {selectedRegions.length}
//           </span>
//         </div>
//         <ChevronDownIcon
